...
<xsl:stylesheet version="1.0" xmlns="http://schema.slothsoft.net/mtg"
	xmlns:html="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:func="http://exslt.org/functions" xmlns:str="http://exslt.org/strings" xmlns:php="http://php.net/xsl"
	xmlns:mtg="http://slothsoft.net/MTG/" extension-element-prefixes="func str php mtg">
	...
	<xsl:key name="cardValue" match="*[@class = 'value']"
		use="normalize-space(preceding-sibling::*[@class = 'label'])" />
	...
	<func:function name="mtg:getCardValue">
		...
		<func:result
			select="php:function('trim', mtg:getNodeText(mtg:getCardNode($valueName, $cardName, $countLimit)/node()))" />
	</func:function>
	...
	<func:function name="mtg:getCardNode">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="ownerCard"
					select="key('cardValue', $LABEL_NAME)[contains($cardName, normalize-space(.))]" />
				<xsl:variable name="valueNode"
					select="key('cardValue', $valueName)[ancestor::html:td/@id = $ownerCard/ancestor::html:td/@id]" />
	...
	<xsl:template name="createCard">
		...
		<card>
			...
			<image>
				<xsl:value-of select="//*[@alt = $cardName]/@src" />
			</image>
			<type>
			...
			<!-- <cmc> <xsl:value-of select="mtg:getCardValue($LABEL_CMC, $cardName)"/> </cmc> -->
			<description>

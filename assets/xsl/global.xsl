...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml"
	...
	<func:function name="mtg:card-has-all-colors">
		...
		<func:result
			select="($t[1]='0' or $c[1]='1') and ($t[2]='0' or $c[2]='1') and ($t[3]='0' or $c[3]='1') and ($t[4]='0' or $c[4]='1') and ($t[5]='0' or $c[5]='1')" />
	</func:function>
		...
		<header style="display: none;">
			<xsl:for-each select="$rootPage/ancestor-or-self::page[page]">
				<nav>
					<details>
						<summary data-dict="">
							<xsl:value-of select=".//*[descendant-or-self::*[@current]]/@title" />
						</summary>
						<div>
							<ul>
					...
					<!-- <div class="details" tabindex="0"> <div class="summary" data-dict=""><xsl:value-of select=".//*[descendant-or-self::*[@current]]/@title"/></div> 
						<ul> <xsl:for-each select="page"> <li> <xsl:if test="descendant-or-self::*[@current]"> <xsl:attribute name="class">active</xsl:attribute> 
						</xsl:if> <a href="{@uri}" data-dict=""> <xsl:value-of select="@title"/> </a> </li> </xsl:for-each> </ul> </div> -->
				</nav>
			</xsl:for-each>
		</header>
		<!-- <div class="header"> <xsl:for-each select="$rootPage/ancestor-or-self::page[page]"> <nav> <xsl:for-each select="page"> 
			<a href="{@uri}" data-dict=""> <xsl:if test="descendant-or-self::*[@current]"> <xsl:attribute name="class">active</xsl:attribute> 
			</xsl:if> <xsl:value-of select="@title"/> </a> </xsl:for-each> </nav> </xsl:for-each> </div> -->
	</xsl:template>
						...
						<xsl:variable name="name">
							<xsl:choose>
								<xsl:when test="substring-after(@name, ' &amp; ')">
									...
									<!-- <xsl:text> (</xsl:text> <xsl:value-of select="normalize-space(substring-before(@name, '&amp;'))"/> <xsl:text>)</xsl:text> -->
								</xsl:when>
								<xsl:otherwise>
	...
	<xsl:template name="card">
		...
		<xsl:for-each select="$card">
			<article data-card-rarity="{@rarity}" data-card-name="{@name}" data-card-stock="{@stock}">
				<!-- <xsl:variable name="colorList" select="color[. != 'Colorless']"/> <xsl:if test="count($colorList)"> <xsl:attribute 
					name="style"> <xsl:text>background: linear-gradient(to right</xsl:text> <xsl:for-each select="$colorList"> <xsl:value-of 
					select="concat(', var(', ., ')')"/> </xsl:for-each> <xsl:if test="count($colorList) = 1"> <xsl:value-of select="concat(', 
					var(', $colorList[1], ')')"/> </xsl:if> <xsl:text>)</xsl:text> </xsl:attribute> </xsl:if> -->
		...
		<xsl:for-each select="$card">
			<img alt="{@name}&#10;{@type}&#10;{@description}" data-src="/slothsoft@{substring-after(@href-image, '/getData.php/')}">
				...
				<!-- <xsl:attribute name="data-src"><xsl:value-of select="@href-image"/></xsl:attribute> <xsl:attribute name="onmouseenter">if 
					(!this.src) this.src = this.getAttribute('data-src');</xsl:attribute> -->
	...
	<xsl:template name="card.href">
		<xsl:choose>
			<xsl:when test="@href-oracle">
				<a href="{@href-oracle}" target="_blank">
					<xsl:value-of select="@name" />
				</a>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template name="card.actions">
		...
		<xsl:for-each select="$card">
			<nav class="clicked-show">
				...
				<!-- <span class="clicked-show"> <a href="/getFragment.php/mtg/print?uri={@href-image}">⎙</a> </span> -->
				<xsl:choose>
					...
					<xsl:when test="@stock">
						<code class="button">
							<input type="text" pattern="-?\d+" required="required" value="{@stock}" name="stock[{name(..)}][{@name}]"
								tabindex="1" title="number of these cards in the deck">
	...
	<xsl:template name="search.input">
		...
		<input type="search" name="{$key}[name]" value="{$search/@query-name}" title="Name" placeholder="Name..."
			autofocus="autofocus" />
		<input type="search" name="{$key}[type]" value="{$search/@query-type}" title="Type" placeholder="Type..." />
		<input type="search" name="{$key}[rarity]" value="{$search/@query-rarity}" title="Rarity"
			placeholder="Rarity..." />
		<input type="search" name="{$key}[expansion_name]" value="{$search/@query-expansion_name}" title="Expansion"
			placeholder="Expansion..." />
		<input type="search" name="{$key}[description]" value="{$search/@query-description}" title="Rules Text"
			placeholder="Rules Text..." />
		<input type="search" name="{$key}[flavor]" value="{$search/@query-flavor}" title="Flavor Text"
			placeholder="Flavor Text..." />
		<input type="search" name="{$key}[legality]" value="{$search/@query-legality}" title="Format"
			placeholder="Format..." />
		<input type="search" name="{$key}[cmc]" value="{$search/@query-cmc}" title="Converted Mana Cost"
			placeholder="CMC..." />
		<input type="search" name="{$key}[colors]" value="{$search/@query-colors}" title="Number of Colors"
			placeholder="Number of Colors..." />
		<input type="search" name="{$key}[price_gt]" value="{$search/@query-price_gt}" title="Market Price (€)"
	...
	<xsl:template name="search.input.color">
		...
		<label>
			<img data-src="/slothsoft@mtg/images/color.{php:function('strtolower', $color)}" alt="{$color}" />
	...
	<xsl:template name="deck.mana">
		...
		<!-- <fieldset class="colors"> <legend>Color Distribution</legend> <table> <caption></caption> <thead> <tr> <th/> <th data-category="Common">Total</th> 
			<xsl:for-each select="$colorList"> <th data-category="{.}"><xsl:value-of select="."/></th> </xsl:for-each> </tr> </thead> 
			<tbody> <tr> <th>Cards</th> <td data-category="Common"><xsl:value-of select="sum($filterList/@stock)"/></td> <xsl:for-each 
			select="$colorList"> <td data-category="{.}"> <xsl:value-of select="sum($filterList[color[. = current()]]/@stock)"/> </td> 
			</xsl:for-each> </tr> <tr> <th>Mana</th> <td data-category="Common"><xsl:value-of select="sum($filterList/color/@val)"/></td> 
			<xsl:for-each select="$colorList"> <td data-category="{.}"> <xsl:value-of select=" 1 * sum($filterList[@stock = 1]/color[. 
			= current()]/@val) + 2 * sum($filterList[@stock = 2]/color[. = current()]/@val) + 3 * sum($filterList[@stock = 3]/color[. 
			= current()]/@val) + 4 * sum($filterList[@stock = 4]/color[. = current()]/@val) + 5 * sum($filterList[@stock = 5]/color[. 
			= current()]/@val) + 6 * sum($filterList[@stock = 6]/color[. = current()]/@val) + 7 * sum($filterList[@stock = 6]/color[. 
			= current()]/@val) + 8 * sum($filterList[@stock = 6]/color[. = current()]/@val)"/> </td> </xsl:for-each> </tr> </tbody> </table> 
			</fieldset> -->
		<!-- <table border="1"> <caption>Color Distribution</caption> <thead> <tr> <th/> <th>Total</th> <th>Colorless</th> <xsl:for-each 
			select="$colorList"> <th><xsl:value-of select="@name"/></th> </xsl:for-each> </tr> </thead> <tbody> <tr> <th>Cards</th> <th><xsl:value-of 
			select="sum($cardList/@stock)"/></th> <th><xsl:value-of select="sum($cardList[mana[not(@color)]]/@stock)"/></th> <xsl:for-each 
			select="$colorList"> <th><xsl:value-of select="sum($cardList[mana[@color = current()/@name]]/@stock)"/></th> </xsl:for-each> 
			</tr> <tr> <th>Mana</th> <th><xsl:value-of select="sum($cardList/mana/@val)"/></th> <th> <xsl:value-of select=" 1 * sum($cardList[@stock 
			= 1]/mana[not(@color)]/@val) + 2 * sum($cardList[@stock = 2]/mana[not(@color)]/@val) + 3 * sum($cardList[@stock = 3]/mana[not(@color)]/@val) 
			+ 4 * sum($cardList[@stock = 4]/mana[not(@color)]/@val) + 5 * sum($cardList[@stock = 5]/mana[not(@color)]/@val) + 6 * sum($cardList[@stock 
			= 6]/mana[not(@color)]/@val) + 7 * sum($cardList[@stock = 7]/mana[not(@color)]/@val) + 8 * sum($cardList[@stock = 8]/mana[not(@color)]/@val) 
			+ 9 * sum($cardList[@stock = 9]/mana[not(@color)]/@val)"/> </th> <xsl:for-each select="$colorList"> <th> <xsl:value-of select=" 
			1 * sum($cardList[@stock = 1]/mana[@color = current()/@name]/@val) + 2 * sum($cardList[@stock = 2]/mana[@color = current()/@name]/@val) 
			+ 3 * sum($cardList[@stock = 3]/mana[@color = current()/@name]/@val) + 4 * sum($cardList[@stock = 4]/mana[@color = current()/@name]/@val) 
			+ 5 * sum($cardList[@stock = 5]/mana[@color = current()/@name]/@val) + 6 * sum($cardList[@stock = 6]/mana[@color = current()/@name]/@val)"/> 
			</th> </xsl:for-each> </tr> </tbody> </table> -->
	</xsl:template>
</xsl:stylesheet>